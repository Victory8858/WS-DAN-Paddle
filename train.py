...
# scheduler = paddle.optimizer.lr.ReduceOnPlateau(learning_rate=0.001, mode='min', patience=3, factor=0.1)
# sgd_optimizer = paddle.optimizer.SGD(learning_rate=scheduler, parameters=net.parameters())
# scheduler = paddle.optimizer.lr.StepDecay(learning_rate=0.001, step_size=2, gamma=0.9)
# sgd_optimizer = paddle.optimizer.SGD(learning_rate=scheduler, parameters=net.parameters(), weight_decay=1e-5)
# adam_optimizer = paddle.optimizer.Adam(learning_rate=scheduler, parameters=net.parameters(), weight_decay=1e-5)
...
#              format(config.epochs, config.batch_size, len(train_dataset), len(val_dataset)))
# logging.info('')
...
#         # end of this batch
#         batch_info = 'Loss {:.4f}, Raw Acc ({:.2f}, {:.2f}), Crop Acc ({:.2f}, {:.2f}), Drop Acc ({:.2f}, {:.2f})'.format(
#             epoch_loss, epoch_raw_acc[0], epoch_raw_acc[1], epoch_crop_acc[0], epoch_crop_acc[1], epoch_drop_acc[0],
#             epoch_drop_acc[1])
#         pbar.update()
#         pbar.set_postfix_str(batch_info)
#
...
#     logs['lr'] = optimizer.get_lr()
#
#     logging.info('Epoch {:03d}, Learning Rate {:g}'.format(epoch + 1, optimizer.get_lr()))
#     pbar = tqdm(total=len(train_loader), unit=' batches')
#     pbar.set_description('Epoch {}/{}'.format(epoch + 1, config.epochs))
#
#     train(logs=logs,
